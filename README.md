...
- **小遊戲 (mini games)** in 紀錄 (`public/lib/arcade.mjs`): play money for the practice account earned by effort, with no luck and no math, four to choose from:
  - 打工輸入號碼 (data entry): key ten-digit numbers off a lottery ticket on the page's own number pad (no phone keyboard; a real keyboard works too; the tenth digit sends it), 8 a round, NT$3 each;
  ...
  - 全壘打大賽 (home run derby), drawn on a canvas: 18 pitches, faster and faster through the round, change-ups after the first fifth and breaking balls from about halfway; a home run (NT$5) needs a swing within about 30 ms and shows its distance, a hit NT$3;
  - 罰球 (free throws): 18 shots; stop the sweeping arrow in the green, which narrows as the arrow speeds up and moves fastest through the green; the ball arcs to the hoop, swishes (NT$5), rims in (NT$3) or bounces off.
