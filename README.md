...
- **小遊戲 (mini games)** in 紀錄 (`public/lib/arcade.mjs`): play money for the practice account earned by effort, with no luck and no math, four to choose from:
  ...
  - 整理彩券 (ticket sorting): each ticket shows a team's logo and name; put it in its league's box (keys 1–4). A round is either the four baseball leagues (is it NPB or KBO?) or NBA, B.League, EuroLeague and the Premier League; NT$1 each;
  - 全壘打大賽 (home run derby), drawn on a canvas: the pitch comes in from the mound, faster each time and sometimes a change-up; a home run (NT$4) needs a swing within about 40 ms and shows its distance, a hit NT$1;
