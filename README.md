...
- **MLB totals (大小分)**: like the lottery, three lines per game: the half-run line closest to 50/50 plus one run either side. DraftKings gives one line, so total runs are modelled as negative binomial (dispersion 5) fitted to it, and the other lines are read off that. Checked on 12 real lottery prices from 2 games (2026-09-25): about 1 percentage point off on average; the lottery's cut on totals is the same ~13%.
