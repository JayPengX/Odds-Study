...
- **小遊戲 (mini games)** in 紀錄 (`public/lib/arcade.mjs`): play money for the practice account earned by effort, with no luck and no math, four to choose from:
  - 打工輸入號碼 (data entry): type ten-digit numbers off a lottery ticket, NT$3 each (20 a round);
  - 整理彩券 (ticket sorting): each ticket names a league; tap its sport's box (or keys 1–4), NT$18 for all 30;
  - 全壘打大賽 (home run derby), drawn on a canvas: the pitch comes in from the mound, faster each time and sometimes a change-up; a home run (NT$4) needs a swing within about 40 ms and shows its distance, a hit NT$2;
  - 罰球 (free throws): stop the sweeping arrow in the green, which narrows as the arrow speeds up; the ball arcs to the hoop, swishes (NT$3), rims in (NT$2) or bounces off.
  Each shows its progress, the round's money and your streak. The pay is balanced: every game pays about NT$25 a minute of typical play (`PACE` in `arcade.mjs`: a round's usual length and an ordinary player's results; a test keeps all four within 15% of it), so none is the one to farm; practice pays more at the two skill games. All of them pay at most NT$1,500 a Taiwan day. After every round the page shows what it came to an hour against Taiwan's minimum wage (NT$196) and how much betting loses it again on average (the lottery keeps about 22%), as a reminder of how slowly money is earned. Winnings are ledger entries like the grants: they sync and merge the same way, and the account's betting result leaves them out.
